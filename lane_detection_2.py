...
canny_lthreshold = 70  # Canny edge detection low threshold 边缘检测下界
canny_hthreshold = 150  # Canny edge detection high threshold 边缘检测上界

# Hough transform parameters 霍夫变换参数
rho = 1 #以像素为单位的距离精度
theta = np.pi / 180#以弧度为单位的角度精度
threshold = 15#阈值
min_line_length = 60
max_line_gap = 30
...
def roi_mask(img, vertices):
  ...
  if len(img.shape) > 2:
    channel_count = img.shape[2]  # i.e. 3 or 4 depending on your imageimage.shape[0], 图片垂直尺寸
